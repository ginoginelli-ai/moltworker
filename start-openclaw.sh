...
if (process.env.OPENAI_API_KEY && process.env.OPENAI_BASE_URL) {
  const modelId = process.env.OPENAI_MODEL || 'route-llm';
  
  // Create the Abacus provider
  config.providers['openai-compatible'] = {
    apiBaseUrl: process.env.OPENAI_BASE_URL,
    apiKey: process.env.OPENAI_API_KEY
  };
  
  // Set Abacus as the primary model
  config.primaryModel = `openai-compatible/${modelId}`;
  console.log('Successfully patched for Abacus RouteLLM');
}
